...
static int rr_net_create_socket(char *err, int domain) {
    ...
    if (rr_net_reuseaddr(err,s) == RR_ERROR) {
        close(s);
        return RR_ERROR;
    }
    return s;
...
int rr_net_tcpserver(char *err, int port, char *bindaddr, int af, int backlog) {
    ...
    for (p = servinfo; p != NULL; p = p->ai_next) {
        ...
        if (af == AF_INET6 && rr_net_only_inet6(err,s) == RR_ERROR) goto error;
        if (rr_net_reuseaddr(err,s) == RR_ERROR) goto error;
        if (rr_net_listen(err, s, p->ai_addr, p->ai_addrlen, backlog) == RR_ERROR) goto error;
        goto end;
    }
    if (p == NULL) {
